...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        print('#######nmt p_shape', P.size())
        print('#######nmt target_padded_shape', target_padded.size())
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded) #(s, b, e)
        X = pack_padded_sequence(X, source_lengths) #(sum_of_lengths, b, e)

        enc_hiddens,(last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens) #(s, b, 2h)
        enc_hiddens = enc_hiddens.transpose(0, 1)  #(b, s, 2h)

        last_hidden = torch.cat((last_hidden[0], last_hidden[1]),1) #(2, b, h) -> (b, 2h) 2 is forthe two hidden states of forward and backward rnn
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###     2. Construct tensor `Y` of target sentences with shape Cusing the target model embeddings.
        ...
        for Y_t in torch.split(Y, 1, 0):  #(1, b, e)
            squeeze = torch.squeeze(Y_t, 0) #(b, e)
            Ybar_t = torch.cat((squeeze, o_prev), dim=1) #(b, e+h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        att_t = torch.unsqueeze(alpha_t,1)
        a_t = torch.bmm(att_t, enc_hiddens).squeeze(1)
