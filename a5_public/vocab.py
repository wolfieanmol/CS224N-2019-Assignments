...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.start_of_word]+[self.char2id[char] for char in list(word)] + [self.end_of_word] for word in sent] for sent in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sen_list = pad_sents_char(self.words2charindices(sents), self.char2id['<pad>'])
        sen_tensor = torch.tensor(sen_list, dtype=torch.long, device=device)
        return torch.transpose(sen_tensor, 0, 1)
